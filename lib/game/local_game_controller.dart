...
class LocalGameController extends ChangeNotifier {
  ...
  void _rollForCurrentPlayer() {
    ...
    if (legalTokens.isEmpty) {
      _botTimer = Timer(const Duration(milliseconds: 650), _passTurn);
    } else if (isBotTurn) {
  ...
  void move(int tokenId) {
    if (isBotTurn || !legalTokens.contains(tokenId)) return;
    _applyMove(tokenId);
  }

  void _playBotMove() {
    if (!isBotTurn || state.dice == null) return;
  ...
  void _startBotIfNeeded() {
    if (state.winnerId == null && isBotTurn && state.dice == null) {
      _botTimer?.cancel();
      _botTimer = Timer(const Duration(milliseconds: 500), _rollForCurrentPlayer);
    }
  }

  void _passTurn() {
    if (state.dice == null || state.winnerId != null) return;
  ...
  void restart() {
    _botTimer?.cancel();
    state = LudoRules.newGame(
      playerCount: state.players.length,
      botPlayers: {for (final p in state.players.where((p) => p.kind == PlayerKind.bot)) p.id},
    );
    notifyListeners();
    _startBotIfNeeded();
  }
