...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(body: SafeArea(child: Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    const Spacer(),
    Container(width: 76, height: 76, decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppTheme.green, Color(0xFF0AAE79)]), borderRadius: BorderRadius.circular(24)), child: const Icon(Icons.casino, color: Colors.white, size: 42)),
    ...
    const SizedBox(height: 8), Text(context.l10n.text('tagline'), style: Theme.of(context).textTheme.titleMedium?.copyWith(color: Colors.black54)),
    const SizedBox(height: 40),
    SizedBox(width: double.infinity, child: ElevatedButton.icon(onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const PlayerSelectionScreen())), icon: const Icon(Icons.play_arrow), label: Text(context.l10n.text('localGame')))),
    ...
    const Spacer(), Center(child: Text('DZ • PLAY WITH PRIDE', style: Theme.of(context).textTheme.labelMedium?.copyWith(letterSpacing: 2, color: AppTheme.red, fontWeight: FontWeight.bold))),
  ])));
